...
public class CustomStainedGlassDyeingRecipeFiller implements CraftingRecipeFiller<CustomStainedGlassDyeingRecipe> {
    @Override
    public Collection<Display> apply(RecipeEntry<CustomStainedGlassDyeingRecipe> recipe) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            ItemStack output = UnidyeUtils.blendAndSetColor(new ItemStack(UnidyeBlocks.CUSTOM_STAINED_GLASS), dyeItems, Lists.newArrayList());
            output.setCount(8);
            for (int k = 0; k < 9; k++) {
                if (k != 4) {
    ...
    @Override
    public Class<CustomStainedGlassDyeingRecipe> getRecipeClass() {
        return CustomStainedGlassDyeingRecipe.class;
    }
}
