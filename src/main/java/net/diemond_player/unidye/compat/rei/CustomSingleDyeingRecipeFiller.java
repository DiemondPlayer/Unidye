...
import net.diemond_player.unidye.recipe.CustomSingleDyeingRecipe;
...
public class CustomSingleDyeingRecipeFiller implements CraftingRecipeFiller<CustomSingleDyeingRecipe> {

    private final ItemConvertible itemOutput;
    private final TagKey<Item> itemTag;
    private final ArrayList<Item> acceptedItems;

    public CustomSingleDyeingRecipeFiller(TagKey<Item> itemTag, ItemConvertible itemOutput) {
    ...
    public CustomSingleDyeingRecipeFiller(ArrayList<Item> acceptedItems, ItemConvertible itemOutput) {
        this.itemOutput = itemOutput;
        this.itemTag = null;
        this.acceptedItems = new ArrayList<>(acceptedItems.stream().filter(item1 -> item1 != itemOutput.asItem()).toList());
    }

    @Override
    public Collection<Display> apply(RecipeEntry<CustomSingleDyeingRecipe> recipe) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (DyeItem dyeItem : dyeItems) {
                inputs.add(EntryIngredient.of(EntryStack.of(VanillaEntryTypes.ITEM, dyeItem.getDefaultStack())));
            }
    ...
    @Override
    public Class<CustomSingleDyeingRecipe> getRecipeClass() {
        return CustomSingleDyeingRecipe.class;
    }
}
